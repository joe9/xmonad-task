...
sendCommand' addr s
  | isPrefixOf "TaskCommand " s =
      sendTaskCommand addr s
      -- . (\t -> readNote ("could not read to Task: " ++ t) t)
      . readMay
      . drop (length "TaskCommand ")
      $ s
  | otherwise = sendCommand addr s
-- sendCommand' addr ("TaskCommand ":xs) = undefined
--     -- sendTaskCommand addr . (readMay :: String -> Maybe Task) $ xs
-- sendCommand' addr s  = sendCommand addr s

sendTaskCommand :: String -> String -> Maybe Task -> IO ()
sendTaskCommand _ rawt Nothing =
  error $ "could not read task from string: " ++ rawt
sendTaskCommand addr _ (Just t) =
  sendCommand addr $ "TaskCommand " ++ show t
